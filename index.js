    ...
    request.get(options).then($ => {
      $("#searchResults li.article").each(function() {
        var productML = $(this)
          ...
          .attr("id");
        var url = $(this)
          .find(".item__info-link")
          .attr("href");
        var name = $(this)
          .find(".list-view-item-title")
          .text();
        var shipping = $(this)
          .find("div.item__shipping > p")
          .text();
        var decimal = $(this)
          .find("div.item__price > span.price__decimals")
          .text();
        var price = $(this)
          .find("div.item__price > span.price__fraction")
          .text();
        price =
          price.replace(".", "") + (decimal != "" ? "." + decimal : ".00");
        // Verifica se a url já existe e adiciona
        modelProducts.findOne({ where: { productML } }).then(product => {
          if (product == null) {
            let newProduct = {
              productML,
              url,
